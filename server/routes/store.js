...
router.post("/register", async (req, res) => {
  const { country, number, code, password } = req.body;
  if (!country || !number || !code || !password) {
    return res.status(422).json({ error: "Please fill all the fields." });
  }
  try {
    const userSearchByNumber = Store.findOne({ number: number });
    if (userSearchByNumber) {
      return res.status(422).json({ error: "store already exists." });
    }
    const store = new Store({ country, number, code, /* sname, */ password });
    res.status(201).json({ message: "Registered Successfully!", store: store });

    const registered = await store.save();
  } catch (error) {
    res.status(500).json({ message: `Could not create store! --> ${error}` });
  }
});
